    ...
    <>
      <main className="flex align-top relative top-10 gap-8">
          ...
          <div className="flex justify-between w-full">
            ...
            <div>
              <p className="font-normal text-gray-600">
                <b className=" text-black font-extrabold">Invoice No:</b>
                {InvoiceInformation[0].InvNo}
              </p>
              <p className="font-normal text-gray-600">
                <b className=" text-black font-extrabold">Date:</b>
                {InvoiceInformation[0].estimatedDeliveryDate}
              </p>
            </div>
          </div>
        ...
        <tbody className="bg-white border-x-2 border-y-2 border-black">
            ...
            <tr key={invoice.itemRef} className=" border-x-2 border-y-2 border-black">
                ...
                <td className="px-4 py-2 text-left text-nowrap text-secondary text-sm font-bold font-manrope border-x-2 border-y-2 border-black">
                {invoice.amount * invoice.quantity}
                </td>
                <td className="px-4 py-2 text-left text-nowrap text-secondary text-sm font-bold font-manrope border-x-2 border-y-2 border-black">
                {invoice.tax}
                </td>
                <td className="px-4 py-2 text-left text-nowrap text-secondary text-sm font-bold font-manrope border-x-2 border-y-2 border-black">
                {invoice.amount * invoice.quantity + (invoice.tax * invoice.amount * invoice.quantity)}
                </td>
            </tr>        
            ))}
            <tr>
                ...
                <td className="px-4 py-2 text-left text-nowrap text-secondary text-sm font-bold font-manrope border-x-2 border-y-2 border-black">
                {InvoiceItems.reduce((acc, item) => acc + item.amount * item.quantity + (item.tax * item.amount * item.quantity), 0)}
                </td>
            </tr>
